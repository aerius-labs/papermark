...
export default function DomainSection({
  ...
  const [displayValue, setDisplayValue] = useState<string>("dataroom.verifiable.exchange");
  ...
  const isEditingCustomDomain =
    editLink && data.domain && data.domain !== "dataroom.verifiable.exchange" ? true : false;
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (isEditingCustomDomain && !canChangeCustomDomain) {
      setDisplayValue(data.domain ?? "dataroom.verifiable.exchange");
      return;
    }
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      setModalOpen(true);
      setData({ ...data, domain: "dataroom.verifiable.exchange" });
      setDisplayValue("dataroom.verifiable.exchange");
      return;
    }

    // Check if this is a custom domain selection (not papermark.com)
    if (value !== "dataroom.verifiable.exchange") {
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      const domainValue = canUseCustomDomain
        ? (defaultDomain?.slug ?? "dataroom.verifiable.exchange")
        : "dataroom.verifiable.exchange";
  ...
  const defaultDomain = editLink
    ? (data.domain ?? "dataroom.verifiable.exchange")
    : (linkType === "DOCUMENT_LINK" && canUseCustomDomainForDocument) ||
        (linkType === "DATAROOM_LINK" && canUseCustomDomainForDataroom)
      ? (domains?.find((domain) => domain.isDefault)?.slug ?? "dataroom.verifiable.exchange")
      : "dataroom.verifiable.exchange";
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "dataroom.verifiable.exchange"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="dataroom.verifiable.exchange" className="hover:bg-muted">
              papermark.com
            </SelectItem>
            {linkType === "DOCUMENT_LINK" && (
              <>
        ...
        {data.domain && data.domain !== "dataroom.verifiable.exchange" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "dataroom.verifiable.exchange" ? "flex" : "",
      ...
      {data.domain && data.domain !== "dataroom.verifiable.exchange" && !isDomainVerified ? (

...
export const sendEmail = async ({
  ...
  const fromAddress =
    from ??
    (marketing
      ? "Marc from Papermark <marc@ship.dataroom.verifiable.exchange>"
      : system
        ? "Papermark <system@dataroom.verifiable.exchange>"
        : verify
          ? "Papermark <system@verify.dataroom.verifiable.exchange>"
          : !!scheduledAt
            ? "Marc Seitz <marc@dataroom.verifiable.exchange>"
            : "Marc from Papermark <marc@dataroom.verifiable.exchange>");
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      cc: cc,
      replyTo: marketing ? "marc@dataroom.verifiable.exchange" : replyTo,
      subject,
      react,
      scheduledAt,
      text: plainText,
